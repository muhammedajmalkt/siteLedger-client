...
export default function SpendingFormModal({ isOpen, setIsOpen, siteId, spending, onSuccess }) {
  const [title, setTitle] = useState("")
  const [description, setDescription] = useState("")
  const [amount, setAmount] = useState("")

  const isEditMode = !!spending

  // Prefill form when editing
  useEffect(() => {
    if (isOpen && spending) {
      console.log("Prefilling spending form with:", spending)
  ...
  const createSpending = async (url, { arg }) => {
    return spendingsAPI.create(siteId, arg)
  }

  const updateSpending = async (url, { arg }) => {
    return spendingsAPI.update(siteId, spending.id, arg)
  }

  const { trigger: triggerCreate, isMutating: creating } = useSWRMutation(`/spendings/${siteId}`, createSpending)
  const { trigger: triggerUpdate, isMutating: updating } = useSWRMutation(`/spendings/${siteId}/${spending?.id}`, updateSpending)
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // Refresh data
      await mutate(`/spendings/${siteId}`)
      await mutate(`/sites/${siteId}`) // Refresh site data to update totals
      onSuccess?.()
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="relative w-full max-w-md rounded-2xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 border border-white/20 p-6 shadow-2xl">
        ...
        <h3 className="mb-5 text-xl font-semibold text-white text-center">
          {isEditMode ? "Edit Spending" : "Add Spending"}
        </h3>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Purchased 20,000 red bricks"
              rows={3}
              value={description}
          ...
          >
            ...
            {creating || updating ? (isEditMode ? "Updating..." : "Creating...") : (isEditMode ? "Update Spending" : "Add Spending")}
          </button>
        </form>
      </div>
    </div>
